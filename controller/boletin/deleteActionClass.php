  ...
  public function execute() {
    try {
      if (request::getInstance()->isMethod('POST') and request::getInstance()->isAjaxRequest()) {
        $id = request::getInstance()->getPost(cabeceraBoletinTableClass::getNameField(cabeceraBoletinTableClass::ID_CABECERA_BOLETIN, true));

        $ids = array(
            cabeceraBoletinTableClass::ID_CABECERA_BOLETIN => $id
        );
        cabeceraBoletinTableClass::delete($ids, true);
        ...
        $this->defineView('delete', 'boletin', session::getInstance()->getFormatOutput());
      } else {
